...
def Num_Process():
    ...
    # 返回列表
    return split_num, province_name, ID_addr


def Init_Config(split_num, province_name, ID_addr):
    try:
        out_filename = 'phonesection.txt'
        ...
        for num in split_num:
            ...
            split_phone_num = "%s|%s|%s" % (ID_addr_info, phone_num_province, num)
            with open(out_filename, 'a') as f:
                print(split_phone_num, file=f)
        print('phonesection.txt文件生成成功！')
...
if __name__ == '__main__':
    # 格式化号段处理信息
    split_num, province_name, ID_addr = Num_Process()
    Init_Config(split_num, province_name, ID_addr)
